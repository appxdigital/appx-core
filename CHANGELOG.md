...
> **Keep `forbidNonWhitelisted: true` on (fail loud) — do NOT run `whitelist` in strip-only mode on writes.** The generated DTOs cover **flat** fields (scalars + FK ids), not relation navigation fields. A `POST`/`PUT` body that carries nested relation writes (`{ items: { create: [...] } }`, `connect`, `set`, …) will be **rejected with `400`** under `forbidNonWhitelisted` — which is intended: nested writes through generic CRUD are **not** authorized by ABAC (a nested `create`/`connect` bypasses the create rules for the related model), so they should not be accepted here. If your app currently relies on nested writes via CRUD, move them to an explicit controller/service method where you apply the authorization yourself. **Never** silently strip on writes (that drops the nested payload with no error — silent data loss). See [`docs/dtos.md`](../docs/dtos.md#relations--nested-writes).
